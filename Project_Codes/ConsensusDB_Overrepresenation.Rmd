...
pacman::p_load("readr","tidyverse", "openxlsx", "stringr", "ontologyIndex", "biomaRt",
               "data.table","BiocManager","devtools", "archive", "shiny")
...
while(j <= nrow(Converted)){
    try(Converted$GO_POI[j] <- POI_Go_detection(Converted$Uniprot[j]))->doStuffResults
    if(class(doStuffResults) != "try-error") j <- j + 1
}
