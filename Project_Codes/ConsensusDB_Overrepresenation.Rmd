...
for (i in Converted$Value[(count):nrow(Converted)]){
  ...
  while(n<5){
   print(count)
   df <- try(PP_Go_detection(Converted$Value[3]), silent=TRUE)
  if(!is(df, 'try-error')){
...
```{r}
DT::datatable(Converted[1, "Partners"][[1]])
...
```{r}
DT::datatable(Converted[2, "Partners"][[1]])
```
...
```{r}
DT::datatable(Converted[3, "Partners"][[1]])
```
...
```{r}
DT::datatable(Converted[4, "Partners"][[1]])
...
```{r}
DT::datatable(Converted[5, "Partners"][[1]])
```
...
```{r}
DT::datatable(Converted[6, "Partners"][[1]])
```
...
```{r}
DT::datatable(Converted[7, "Partners"][[1]])
...
```{r}
DT::datatable(Converted[8, "Partners"][[1]])
```
...
```{r}
DT::datatable(Converted[9, "Partners"][[1]])
```
...
```{r}
DT::datatable(Converted[10, "Partners"][[1]])
```
